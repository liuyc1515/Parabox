...
RecursionObject::RecursionObject(std::shared_ptr<const ObjectManager> object_manager, 
                std::shared_ptr<MapManager> map_manager, 
                std::shared_ptr<const std::map<uint64_t, std::shared_ptr<const BaseObject>>> objects) : BaseObject(OBJECT::RECURSION, object_manager, map_manager, objects)
{
    inner_map_id_ = 0;
}
