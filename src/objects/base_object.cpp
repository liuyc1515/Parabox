...
BaseObject::BaseObject(
	OBJECT::ObjectType type, std::shared_ptr<const ObjectManager> object_manager,
...
bool BaseObject::CompareReturnAndExpectedAct(ACTION::Action ret_act, ACTION::Action exp_act) const {
	return ret_act == exp_act || ret_act == exp_act + (ACTION::LEFT_OUT - ACTION::LEFT) ||
		ret_act == exp_act + (ACTION::LEFT_INTO - ACTION::LEFT) || ret_act == ACTION::DISAPPEAR;
}
...
Coordinate BaseObject::CalcCoordInAnotherMap(
	const Coordinate &coord, uint64_t from_map_id, uint64_t &to_map_id, ACTION::Action act
) const {
	...
	if (act >= ACTION::UP_INTO && act <= ACTION::RIGHT_INTO) {
		...
		new_coord = CalcCoordByDirection(
			map_manager_->GetVoidBorder(OppositeDirection(dir), to_map_id), OppositeDirection(dir)
		);
...
std::shared_ptr<const BaseObject> BaseObject::GetAround(ACTION::Action act, uint64_t map_id) const {
	...
	if (act >= ACTION::UP && act <= ACTION::RIGHT) {
		...
		self_coord = CalcCoordInAnotherMap(
			object_manager_->GetObjectCoord(GetID()), object_manager_->GetObjectMap(GetID()), map_id, act
		);
	}
	DIRECTION::Direction dir = ActionToDirection(act);
