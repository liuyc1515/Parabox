    ...
    for (auto obj : *objects_)
    {
        if (obj.second.get() != NULL)
        {
            if (changes.find(obj.second->GetID()) != changes.end())
            {
                ...
                {
                    if (changes.at(obj.second->GetID()) <= ACTION::RIGHT_INTO && changes.at(obj.second->GetID()) >= ACTION::UP_INTO)
                    {
                        // calc the coord of the map object this object will get into
                        tmp_coord = map_manager_->CalcCoordByAction(object_manager_->GetObjectCoord(obj.second->GetID()), (ACTION::Action)(changes.at(obj.second->GetID()) - (ACTION::UP_INTO - ACTION::UP)));
                    ...
                    {
                        tmp_coord = map_manager_->CalcCoordByAction(object_manager_->GetObjectCoord(obj.second->GetID()), changes.at(obj.second->GetID()));
                        tmp_type = obj.second->GetType();
