...
BaseAgent::BaseAgent() : objects_(std::make_shared<std::map<uint64_t, std::shared_ptr<const BaseObject>>>()) {
	...
	for (int i = 0; i < map_manager_->GetMapX(map_manager_->GetCurrentMapID()); ++i) {
		for (int j = 0; j < map_manager_->GetMapY(map_manager_->GetCurrentMapID()); ++j) {
			...
			object_manager_->SetObjectInfo(
				tmp_object->GetID(), {{i, j}, tmp_object->GetType(), map_manager_->GetCurrentMapID()}
			);
			...
			if (tmp_object.get() != NULL) {
				if (changes.find(tmp_object->GetID()) != changes.end()) {
					if (tmp_object->GetType() != OBJECT::VOID) {
						new_map.insert(
							{tmp_object->GetID(),
							 {map_manager_->CalcCoordByAction({i, j}, changes.at(tmp_object->GetID())),
							  tmp_object->GetType(), map_manager_->GetCurrentMapID()}}
						);
					} else {
						new_map.insert(
							{tmp_object->GetID(),
							 {object_manager_->GetObjectCoord(operator_->GetID()), tmp_object->GetType(),
							  map_manager_->GetCurrentMapID()}}
						);
					}
				} else {
					new_map.insert(
